...
export const adminApi = {
  ...
  scraperFlows: () => apiFetch<Array<{ id: string; name: string; startUrl: string; status: string; version: number; lastRunAt: string | null; nextRunAt: string | null; updatedAt: string; stepCount: number; supermarket: { id: string; name: string; slug: string }; latestRun: null | { id: string; status: string; startedAt: string; finishedAt: string | null; flyersFound: number; stepsExecuted: number; error: string | null; log: string | null } }>>("/admin/scraper-flows"),
  ...
  updateOfferCatalog: (offerId: string, data: { brandId?: string | null; canonicalProductId?: string | null }) => apiFetch(`/admin/offers/${offerId}/catalog`, { method: "PATCH", body: JSON.stringify(data) }),
