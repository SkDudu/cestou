...
export const adminApi = {
  ...
  scraperRun: (runId: string) => apiFetch<{ id: string; status: string; startedAt: string; finishedAt: string | null; error: string | null; flow: { name: string; supermarket: { name: string } }; events: Array<{ id: string; sequence: number; type: string; payload: unknown; createdAt: string }> }>(`/admin/scraper-runs/${runId}`),
  ...
  offer: (offerId: string) => apiFetch<{ id: string; name: string; price: string | number; validationStatus: string; supermarket: { name: string }; flyer: { title: string | null }; brandRecord: { id: string; name: string } | null; canonicalProduct: { id: string; canonicalName: string } | null }>(`/admin/offers/${offerId}`),
