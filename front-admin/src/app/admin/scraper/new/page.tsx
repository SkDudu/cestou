...
function NewWorkerForm() {
  ...
  const [supermarkets, setSupermarkets] = useState<Array<{ id: string; name: string; websiteUrl: string | null }>>([]);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (cause instanceof ApiError && cause.code === "SUPERMARKET_WEBSITE_REQUIRED") {
        ...
        setError("Dados inválidos para criar o fluxo. Confira nome e URL da rede.");
  ...
  return (
    <div>
      ...
      <OpsHeader
        title="Novo worker"
        subtitle="Cria o fluxo no Postgres. URL inicial vem do cadastro da rede."
      />
      <form onSubmit={(event) => void submit(event)} className="max-w-xl space-y-4 rounded-[14px] border border-[var(--ds-color-border)] bg-[var(--ds-color-card)] p-5">
        ...
        {selected ? (
          <p className="text-sm text-[var(--ds-color-muted-foreground)]">
            URL inicial:{" "}
            {selected.websiteUrl ? (
              <span className="text-[var(--ds-color-foreground)]">{selected.websiteUrl}</span>
            ) : (
              <>
                ausente —{" "}
        ...
        <div className="flex gap-2">
          <button type="submit" disabled={saving || !supermarketId || !selected?.websiteUrl} className="ds-btn ds-btn--primary disabled:opacity-60">
