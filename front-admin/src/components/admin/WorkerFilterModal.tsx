...
export function matchWorkerFilter(
  ...
  if (f.redes.length && !f.redes.includes(redeOf(w.supermarketName))) {
    return false;
  }
  if (f.storeIds.length && !f.storeIds.includes(w.supermarketId)) return false;
  if (
    f.sources.length &&
    !(w.sourceKinds ?? []).some((k) => f.sources.includes(k))
  ) {
    return false;
  }
  if (f.health.length) {
    const hit = f.health.some((h) => {
      ...
      if (h === "stale") {
        return (
          w.status !== "running" &&
          (!w.lastRunAt || now - w.lastRunAt > DAY)
        );
      }
...
export function WorkerFilterPopover({
  ...
  markets: Array<{ _id: Id<"supermarkets">; name: string }>;
  value: WorkerFilter;
  filterCount: number;
    ...
    return markets
      .filter(
        (m) =>
          ...
          !draft.storeIds.includes(m._id),
      )
      .slice(0, 6);
  }, [markets, storeQ, draft.storeIds]);

  const selectedStores = markets.filter((m) => draft.storeIds.includes(m._id));
      ...
      >
        <PopoverHeader>
          ...
          <PopoverDescription className="text-[13px] leading-[18px] text-[var(--ds-color-muted-foreground)]">
            {n} {n === 1 ? "ativo" : "ativos"}. Status continua nos chips da
            lista.
          </PopoverDescription>
        </PopoverHeader>
              ...
              {redes.map((r) => {
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      setDraft((d) => ({ ...d, redes: toggle(d.redes, r) }))
                    }
                  >
                    {r}
                  </button>
              ...
              {selectedStores.map((m) => (
                <button
                  ...
                  onClick={() =>
                    setDraft((d) => ({
                      ...d,
                      storeIds: d.storeIds.filter((id) => id !== m._id),
                    }))
                  }
                >
                  {m.name}
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    setDraft((d) => ({
                      ...d,
                      health: toggle(d.health, id),
                    }))
                  }
                >
                  <span
                    className={`ds-check-box ${on ? "ds-check-box--on" : ""}`}
                  />
                  {label}
                </button>
              );
            ...
            >
              {(
                ...
                <div key={id} className="flex items-center gap-3">
                  ...
                  <Label
                    htmlFor={`parse-${id}`}
                    className="text-[13px] font-medium text-[var(--ds-color-ink)]"
                  >
                    {label}
                  </Label>
                </div>
