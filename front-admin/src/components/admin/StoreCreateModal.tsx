...
type NetworkType = (typeof NETWORK_TYPES)[number]["value"];
...
export function StoreCreateModal({
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setBusy(true);
    try {
      const created = await adminApi.createSupermarket({
        name,
        websiteUrl: site || undefined,
        networkType: networkType || undefined,
      });
      window.location.href = `/admin/supermarkets/${created.id}`;
    } catch (cause) {
      setError(
        cause instanceof ApiError && cause.status === 401
          ? "Sua sessão expirou."
          : "Não foi possível cadastrar a rede.",
      );
      setBusy(false);
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-[13px] font-medium leading-[18px]">Site dos encartes (opcional)</span>
          <input
            type="text"
            inputMode="url"
            value={websiteUrl}
        ...
        <label className="flex flex-col gap-1.5">
          <span className="text-[13px] font-medium leading-[18px]">Tipo de rede</span>
          <select
            className="ds-input"
            value={networkType}
