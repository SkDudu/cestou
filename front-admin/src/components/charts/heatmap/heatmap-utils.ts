...
const heatmapTooltipDateFmt = new Intl.DateTimeFormat("pt-BR", {
  day: "numeric",
  month: "long",
  year: "numeric",
});
...
/** Tooltip header date — e.g. `20 de janeiro de 2026`. */
export function formatHeatmapTooltipDate(date: Date): string {
  return heatmapTooltipDateFmt.format(date);
}

/** Tooltip weekday line — e.g. `Segunda-feira`. */
export function formatHeatmapTooltipWeekday(date: Date): string {
  const s = heatmapTooltipWeekdayFmt.format(date);
  return s.charAt(0).toUpperCase() + s.slice(1);
}

/** Tooltip contribution line — e.g. `3 contribuições`. */
export function formatHeatmapContributionLabel(
  count: number,
  _date?: Date
): string {
  const word = count === 1 ? "contribuição" : "contribuições";
  return `${count} ${word}`;
}

/** Sunday-first day labels for heatmap row bins (pt-BR). */
