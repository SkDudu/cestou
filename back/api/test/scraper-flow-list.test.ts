...
const credentials = { ADMIN_MASTER_EMAIL: "admin-flow-list@example.test", ADMIN_SEED_PASSWORD: "test-password" };
let supermarketId = "";

describe("admin scraper flow listing", () => {
  beforeAll(async () => {
    await prisma.user.deleteMany({ where: { email: credentials.ADMIN_MASTER_EMAIL } });
    await seedMasterAdmin(prisma, credentials);
    const supermarket = await prisma.supermarket.create({ data: { name: "Flow List", slug: "flow-list", city: "Fortaleza", state: "CE", country: "BR", active: true } });
    supermarketId = supermarket.id;
    const flow = await prisma.scraperFlow.create({ data: { supermarketId, name: "Main flow", startUrl: "https://example.test", status: "ACTIVE", version: 1 } });
    await prisma.scraperRun.create({ data: { flowId: flow.id, status: "SUCCESS", finishedAt: new Date() } });
  });
  ...
  it("lists flows with supermarket and latest run for an admin", async () => {
